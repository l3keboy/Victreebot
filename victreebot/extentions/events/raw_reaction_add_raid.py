...
async def event_raw_reaction_add_raid(
    ...
    if event.emoji_id == instinct_emoji_id:
        if raid.end_time - time.time() > int(raid_timeout):
            ...
            emoji = event.app.cache.get_emoji(instinct_emoji_id) or await event.app.rest.fetch_emoji(guild, instinct_emoji_id)
    ...
    if event.emoji_id == mystic_emoji_id:
        if raid.end_time - time.time() > int(raid_timeout):
            ...
            emoji = event.app.cache.get_emoji(mystic_emoji_id) or await event.app.rest.fetch_emoji(guild, mystic_emoji_id)
            await message.remove_reaction(emoji, user=event.member)
