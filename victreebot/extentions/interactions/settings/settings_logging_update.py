...
async def command_settings_update_logging(
    ...
    action_row_1 = (
        ...
        .add_interactive_button(hikari.ButtonStyle.PRIMARY, "logs_channel", label=SUPPORTED_LANGUAGES.get(language).settings_logging_logs_channel)
        .add_interactive_button(hikari.ButtonStyle.PRIMARY, "general_events", label=SUPPORTED_LANGUAGES.get(language).settings_logging_general_events)
        .add_interactive_button(hikari.ButtonStyle.PRIMARY, "profile_events", label=SUPPORTED_LANGUAGES.get(language).settings_logging_profile_events)
        .add_interactive_button(hikari.ButtonStyle.PRIMARY, "location_events", label=SUPPORTED_LANGUAGES.get(language).settings_logging_location_events)
        .add_interactive_button(hikari.ButtonStyle.PRIMARY, "raid_events", label=SUPPORTED_LANGUAGES.get(language).settings_logging_raid_events)
    )
    action_row_2 = (
        ctx.rest.build_message_action_row()
        .add_interactive_button(hikari.ButtonStyle.PRIMARY, "trade_events", label=SUPPORTED_LANGUAGES.get(language).settings_logging_trade_events)
        .add_to_container()
    )
